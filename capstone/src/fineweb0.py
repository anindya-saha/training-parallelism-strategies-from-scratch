...
Usage:
  python fineweb.py
  python fineweb.py --data_root /path/to/edu_fineweb10B
"""
...
logger = logging.getLogger("fineweb")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Download and tokenize FineWeb-Edu to shards")
    parser.add_argument(
        "--data_root",
        type=str,
...
def configure_logging() -> None:
    """Console + log file under capstone/src/log/fineweb.txt (same log/ convention as train.py)."""
    log_dir = os.path.join(os.path.dirname(__file__), "log")
    os.makedirs(log_dir, exist_ok=True)
    log_path = os.path.join(log_dir, "fineweb.txt")
...
def main() -> None:
    ...
    logger.info("loading dataset HuggingFaceFW/fineweb-edu name=%s split=train", REMOTE_NAME)
