...
def all_to_all_dispatch(
    ...
    input_splits = [(target_rank[sort_indices] == r).sum().item() for r in range(ep_size)]
    ...
    dist.all_to_all_single(output_splits_tensor, input_splits_tensor, group=ep_group)
    ...
    dist.all_to_all_single(recv_tokens, x_sorted, output_splits, input_splits, group=ep_group)

    # Also exchange expert IDs so we know which local expert to run
    recv_expert_ids = torch.empty(total_recv, dtype=expert_ids_sorted.dtype, device=x_flat.device)
    dist.all_to_all_single(recv_expert_ids, expert_ids_sorted, output_splits, input_splits, group=ep_group)
...
def all_to_all_combine(
    ...
    dist.all_to_all_single(recv_back, expert_output, input_splits, output_splits, group=ep_group)
...
class DistributedSparseMoELayer(nn.Module):
    ...
    def __init__(self, config: MoEGPTConfig, ep_size: int, ep_group: dist.ProcessGroup):
        ...
        self.experts = nn.ModuleList([ExpertFFN(config) for _ in range(self.experts_per_rank)])
    ...
    def forward(self, x: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        recv_tokens, sort_indices, in_splits, out_splits, local_expert_ids = all_to_all_dispatch(
            x_expanded,
            expert_indices,
            self.ep_size,
            self.num_experts,
...
class MoEGPT(nn.Module):
    def __init__(self, config: MoEGPTConfig, ep_size: int, ep_group: dist.ProcessGroup):
        ...
        self.blocks = nn.ModuleList([MoETransformerBlock(config, ep_size, ep_group) for _ in range(config.n_layers)])
