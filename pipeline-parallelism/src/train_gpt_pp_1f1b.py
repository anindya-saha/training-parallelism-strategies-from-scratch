"""GPT-2 with hand-written Pipeline Parallelism -- 1F1B (PipeDream-Flush) schedule.

1F1B schedule: after a short warmup of forwards, the steady state interleaves
one backward with one forward. This limits peak stored activations to
(num_warmup + 1) per rank rather than M (the number of microbatches) as in GPipe.

For N stages and rank r:
    num_warmup = pp_size - 1 - rank
    num_steady = n_micro - num_warmup
    num_cooldown = num_warmup

Phase 1 (Warmup):   rank does num_warmup forwards without matching backwards
Phase 2 (Steady):   alternates 1 backward + 1 forward for num_steady iterations
Phase 3 (Cooldown): drains remaining num_cooldown backwards

Peak stored activations on rank r = num_warmup + 1 (during steady state).

Example:
    torchrun --nproc_per_node=2 src/train_gpt_pp_1f1b.py --config mini --pp-size 2
    torchrun --nproc_per_node=4 src/train_gpt_pp_1f1b.py --config mini --pp-size 4 --num-microbatches 8
"""
...
def _do_forward(
    ...
    """Execute one microbatch forward for this rank.

    Returns (saved_output, saved_input, logits_or_none).
    - saved_output: activation output for backward (first/intermediate ranks)
    - saved_input: input requiring grad for backward (intermediate ranks)
    - logits: only on last rank
    """
    if is_first:
        h = stage(micro_input)  # (mbs, T, d_model)
...
def _do_backward(
    rank: int,
    pp_size: int,
    saved_output: torch.Tensor | None,
    saved_input: torch.Tensor | None,
    logits: torch.Tensor | None,
    micro_input: torch.Tensor | None,
    d_model: int,
    device: torch.device,
    is_first: bool,
    is_last: bool,
    ...
    """Execute one microbatch backward for this rank. Returns loss (0 for non-last)."""
    if is_last:
        loss_b = lm_loss(logits, micro_input)
        loss_b.backward()
        assert saved_input.grad is not None
        dist.send(saved_input.grad.detach().contiguous(), dst=rank - 1)
        return loss_b.detach()
    elif is_first:
        grad_h = torch.empty(mbs, T, d_model, device=device)
        dist.recv(grad_h, src=rank + 1)
        saved_output.backward(grad_h)
...
def pipeline_forward_backward_1f1b(
    ...
    """1F1B schedule: warmup forwards, steady (1B + 1F), cooldown backwards.

    Returns loss (avg over microbatches) on last rank, None otherwise.
    """
    ...
    num_warmup = pp_size - 1 - rank
    num_warmup = min(num_warmup, n_micro)
    num_steady = n_micro - num_warmup
    num_cooldown = num_warmup

    # Queues for saved state (FIFO for backward)
    saved_outputs: list[torch.Tensor | None] = []
    saved_inputs: list[torch.Tensor | None] = []
    saved_logits: list[torch.Tensor | None] = []

    fwd_count = 0
    bwd_count = 0
    total_loss = torch.tensor(0.0, device=device)

    # ========== Phase 1: Warmup (forwards only) ==========
    for _ in range(num_warmup):
        out, inp, logits = _do_forward(
            stage, rank, pp_size, micro_inputs[fwd_count],
            d_model, device, is_first, is_last, mbs, T,
        )
    ...
    for _ in range(num_steady):
        ...
        # Backward (oldest microbatch)
        loss_b = _do_backward(
            rank, pp_size,
            saved_outputs[bwd_count],
            saved_inputs[bwd_count],
            saved_logits[bwd_count],
            micro_inputs[bwd_count] if is_last else None,
            d_model, device, is_first, is_last, mbs, T,
        )
        total_loss += loss_b
        bwd_count += 1

    # ========== Phase 3: Cooldown (backwards only) ==========
    for _ in range(num_cooldown):
        loss_b = _do_backward(
            rank, pp_size,
            saved_outputs[bwd_count],
            saved_inputs[bwd_count],
...
def parse_args():
    p = argparse.ArgumentParser(description="GPT-2 Pipeline Parallelism - 1F1B Schedule")
    p.add_argument(
        "--config",
        type=str,
        default="mini",
...
def main():
    ...
    if rank == 0:
        logger.info(
            "GPT-2 Pipeline Parallelism (1F1B) -- config: %s, pp_size: %d, n_micro: %d, mbs: %d",
            args.config,
            pp_size,
            n_micro,
            mbs,
        ...
        logger.info(
            "1F1B: num_warmup=%d (rank 0), peak_activations=%d (rank 0)",
            pp_size - 1,
            pp_size,
        )
    ...
    for step in range(args.benchmark):
        ...
        loss = pipeline_forward_backward_1f1b(
            stage, rank, pp_size, micro_inputs, config.d_model, device
        )
    ...
    if rank == 0:
        ...
        out_path = os.path.join(
            args.output_dir,
            f"results_train_gpt_pp_1f1b_{args.config}_pp{pp_size}_m{n_micro}.json",
        )
        with open(out_path, "w") as fout:
            json.dump(results, fout, indent=2)
        logger.info("=" * 60)
        logger.info(
            "  GPT-2 %s - Pipeline Parallelism (PP=%d, 1F1B, M=%d) - %d GPU(s)",
            args.config.upper(),
            pp_size,
            n_micro,
            world_size,
