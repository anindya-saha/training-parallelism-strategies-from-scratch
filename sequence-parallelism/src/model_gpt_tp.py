"""Vanilla TP GPT-style transformer model (baseline for SP comparison).
...
In vanilla TP:
- Column-parallel layers split the output dimension, no communication in forward
- Row-parallel layers split the input dimension, ALL-REDUCE to combine in forward
- LayerNorm, Dropout, and Residual connections operate on FULL (B, S, h) tensors
  that are identical on every GPU - this is the memory overhead that Sequence
...
# ================================================================
# _CopyToTPRegion:      identity forward,   all-reduce backward  (f in Megatron diagram)
#   -> Used before column-parallel layers
# _ReduceFromTPRegion:  all-reduce forward, identity backward    (f* in Megatron diagram)
...
class RowParallelLinear(nn.Module):
    def __init__(self, d_in: int, d_out: int, bias: bool = True) -> None:
        ...
        nn.init.kaiming_uniform_(self.weight, a=math.sqrt(5))
        # Bias only on rank 0: _ReduceFromTPRegion all-reduces the output,
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        return _ReduceFromTPRegion.apply(out)
...
# ================================================================
# In vanilla TP, LayerNorm, Dropout, and Residual connections all
# operate on the FULL (B, S, h) tensor that is identical on every
# GPU. This is where activation memory is "wasted" - each GPU
# stores the same full-sized activations for these ops.
# Sequence Parallelism (TP+SP) fixes this by scattering the
# sequence dimension so each GPU only holds (B, S/N, h).
...
class TPGPTTransformerBlock(nn.Module):
    def __init__(
        ...
        super().__init__()
        self.norm1 = nn.LayerNorm(d_model)  # FULL (B, S, h) on every GPU
        self.attn = TPAttention(d_model, n_heads, bias=attn_bias, dropout=dropout)
        self.norm2 = nn.LayerNorm(d_model)  # FULL (B, S, h) on every GPU
        self.ffn = TPFFN(d_model, d_ff, bias=ffn_bias)
        self.resid_dropout = nn.Dropout(dropout)  # FULL (B, S, h) on every GPU
...
class TPGPT(nn.Module):
    def __init__(
        ...
        self.emb_dropout = nn.Dropout(dropout)  # FULL (B, S, h) on every GPU
...
def main():
    ...
    if rank == 0:
        ...
        logger.info("=" * 60)
        logger.info("  Model GPT - Tensor Parallelism (baseline for SP) - %d GPUs", ws)
        logger.info("=" * 60)
