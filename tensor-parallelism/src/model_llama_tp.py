"""TP Llama-style transformer model.

A minimal Llama decoder with Tensor Parallelism. Weight matrices are sharded
across GPUs using Column-Parallel and Row-Parallel linear layers.

Differences from the GPT TP model (model_gpt_tp.py):
...
# ================================================================
# TP Grouped Query Attention (split by heads)
# ================================================================
# Q heads are split across GPUs (column-parallel).
# KV heads are also split across GPUs (column-parallel).
# Constraint: n_kv_heads must be divisible by world_size.
# W_o is row-parallel. The cancellation between column and row still applies.


class TPGroupedQueryAttention(nn.Module):
    def __init__(
        self,
        d_model: int,
        n_heads: int,
        ...
        assert n_kv_heads % ws == 0, "n_kv_heads must be divisible by world size"

        self.d_head = d_model // n_heads
        self.n_heads_local = n_heads // ws
        self.n_kv_heads_local = n_kv_heads // ws
        self.group_size = n_heads // n_kv_heads
        self.scale = 1.0 / math.sqrt(self.d_head)

        # Written as n_heads * d_head (== d_model for Q/O) to make the asymmetry with K/V in GQA visible.
        self.W_q = ColumnParallelLinear(d_model, n_heads * self.d_head, bias=bias)
        self.W_k = ColumnParallelLinear(d_model, n_kv_heads * self.d_head, bias=bias)
        self.W_v = ColumnParallelLinear(d_model, n_kv_heads * self.d_head, bias=bias)
        self.W_o = RowParallelLinear(n_heads * self.d_head, d_model, bias=bias)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # Expand local KV heads to match local Q heads (no communication needed)
...
# ================================================================
# TP SwiGLU FFN (feed-forward network)
# ================================================================
# W_gate and W_up are both column-parallel (project up to d_ff).
# W_down is row-parallel (project back to d_model).
# The elementwise SiLU(gate) * up happens locally - both shards are
# split the same way, so no communication needed between them.


class TPSwiGLUFFN(nn.Module):
    def __init__(self, d_model: int, d_ff: int, bias: bool = False):
        super().__init__()
        self.W_gate = ColumnParallelLinear(d_model, d_ff, bias=bias)
        self.W_up = ColumnParallelLinear(d_model, d_ff, bias=bias)
        self.W_down = RowParallelLinear(d_ff, d_model, bias=bias)
...
# ================================================================
# TP Llama transformer block
# ================================================================


class TPLlamaTransformerBlock(nn.Module):
    def __init__(
        self,
        d_model: int,
        n_heads: int,
        ...
        self.attn = TPGroupedQueryAttention(
            d_model, n_heads, n_kv_heads, max_seq_len, bias=attn_bias
        )
        self.norm2 = RMSNorm(d_model)
        self.ffn = TPSwiGLUFFN(d_model, d_ff, bias=ffn_bias)
...
def parse_args():
    p = argparse.ArgumentParser(description="Tensor parallelism Llama benchmark")
...
def main():
    ...
    if rank == 0:
        logger.info("TP Llama benchmark: %d GPUs", ws)
        logger.info(
        ...
        logger.info(
            "Each GPU: %d/%d Q heads, %d/%d KV heads, %d/%d FFN dim",
            args.n_heads // ws,
            args.n_heads,
            args.n_kv_heads // ws,
            args.n_kv_heads,
